...
function &list_categories(){
	...
	while (list($priority, $pname)=$xoopsDB->fetchRow($result)) {
		//利用所屬優先度編號$of_priority取得所屬優先度之名稱
	//	$category_name=get_category_name($of_priority);
		$list.="<tr bgcolor='white'><td>$priority</td><td>$pname</td><td><a href='".$_SERVER['PHP_SELF']."?op=edit&priority=$priority'>編輯</a> | <a href='".$_SERVER['PHP_SELF']."?op=del&priority=$priority'>刪除</a></td></tr>";
	}

	//所有優先度的列表
	$main="<h3>現有需求單優先度名稱列表</h3>
	<table cellspacing='1' cellpadding='3' bgcolor='black'  width=100%>
	<tr bgcolor='#D9E8FF'><td>編號</td><td>優先度名稱</td><td>優先度說明</td><td>功能</td></tr>
	$list
	</table>";
...
//印出相關連結
echo "<a href='".$_SERVER['PHP_SELF']."'>列出需求單的優先度</a> | <a href='".$_SERVER['PHP_SELF']."?op=edit'>新增需求單的優先度</a>";
