...
function save($data=array()){
	...
	$sql_replace = "replace into ".$xoopsDB->prefix("statecat")." (state, sname)  values ('".$data['state']."', '".$data['sname']."')";
...
function &list_categories(){
	...
	$sql_select = "select state, sname from ".$xoopsDB->prefix("statecat")." order by state";
    ...
    <td>$sname</td>
    <td><a href='".$_SERVER['PHP_SELF']."?op=edit&state=$state'>編輯</a> | <a href='".$_SERVER['PHP_SELF']."?op=del&state=$state'>刪除</a></td>
    </tr>";
	}

	//所有狀態的列表    <h3>狀態名稱列表</h3>
	$main="
	<table cellspacing='1' cellpadding='3' bgcolor='black'  width=100%>
	<tr bgcolor='#D9E8FF'><td>需求單狀態名稱</td><td>功能</td></tr>
	$list
	</table>";
...
//印出相關連結
echo "<a href='".$_SERVER['PHP_SELF']."'>列出需求單狀態</a> | <a href='".$_SERVER['PHP_SELF']."?op=edit'>新增需求單狀態</a><br></br>";
