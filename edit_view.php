...
//}else{
  ...
  $sql = "select sn,id,state,category from " . $xoopsDB->prefix('requireform') . " where `id` = '$_POST[id]'";
  ...
  if($state=='4'){						//評估完成
	if($category=='3'){					//UHD單
		...
		if($sign_mis == 'Y'){
			...
			<INPUT type='submit' name='sign_mis' value='情報系統部簽核'>
  ...
  } elseif($state=='7'){	  //情報系統部已簽核 -> 財會小組主管簽核
	if($sign_fin == 'Y'){
...
function show_requireform($sn = "")
{
  ...
  $sql = "select * from " . $xoopsDB->prefix('requireform') . " where `id` = '$_POST[id]'";
  $result = $xoopsDB -> query($sql) or redirect_header("index.php" , 3 , _MD_MYNEWS_GET_NEWS_ERROR."<br />" . $xoopsDB->errno() . " : " . $xoopsDB->error());
...
include_once(XOOPS_ROOT_PATH."/class/xoopsformloader.php");
$form = new XoopsThemeForm("修改欄位", "form", "edit_test.php");
$form->addElement(new XoopsFormText ("修改需求單號", "$id", "10%", 20));
$main3 = $form->render();
