...
function add_requireform($sn = "")
{
  ...
  include_once(XOOPS_ROOT_PATH."/class/xoopsformloader.php");
  $form = new XoopsThemeForm("建立需求單","form", $_SERVER['PHP_SELF'],'post', 'true');
//  $id="AMS-2011";
//  $form->addElement(new XoopsFormText ("需求單編號", "id", "20%", 20, $id),true);
  ...
  $Tray5->addElement(new XoopsFormTextDateSelect ("希望完成日期", "target_date", 15, $target_date),true);
// XOOPS 2.4.5 not support
//  $Tray5->addElement(new XoopsFormCalendar ("希望完成日期", "target_date", 15, $target_date),true); 
  ...
  $form->addElement(new XoopsFormText ("主旨", "headline", "100%", 160, $headline),true);
  if(empty($possible_solutions)) $possible_solutions="請輸入需求內容";
//  $form->addElement(new XoopsFormTextArea ("需求單內容", "new_content", $possible_solutions, 20, 70),true);
